rm(list = ls())
drop<-read.csv("C:/Users/e0265296/My Drive/Topic7_missing labels/2020May JRSSB codes/real data analysis/Drop_test.csv")
myX<-as.matrix(drop[1:20,2:7])
myY<-as.matrix(drop[21:40,2:7])
...
set.seed(150)
dat_aggre3<-list(X=dat_aggre2$X[sample(1:20,20)],Y=dat_aggre2$Y)
test.our(dat_aggre2,m_n = 10)
test.our(dat_aggre2,m_n = 9)
...
dat<-mydat
set.seed(1234)
test.our.multiple(dat=list(X=dat$X[sample(1:20,20),],Y=dat$Y[sample(1:20,20),],Y.match=dat$Y.match),m_n = 10)
...
dat_aggre2<-list(X=my_X,Y=my_Y)
set.seed(1045)
test.our(dat = dat_aggre2,m_n = 10,n_m = 2)
test.our(dat = dat_aggre2,m_n = 11,n_m = 2)
...
dat<-mydat
set.seed(1234)
test.our.multiple(dat = mydat,m_n = 11,n_m = 2)
